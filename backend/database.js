...
const initializeDatabase = async () => {
  ...
  const createTables = `
    ...
    -- Thingsboard設定テーブル
    CREATE TABLE IF NOT EXISTS thingsboard_configs (
      id SERIAL PRIMARY KEY,
...
// Thingsboard設定の保存
const saveThingsboardConfig = async (config) => {
  if (!pool) {
    throw new Error('データベースに接続されていません');
  }

  console.log('Saving Thingsboard config:', {
  ...
  const query = `
    INSERT INTO thingsboard_configs (base_url, username, password)
    VALUES ($1, $2, $3)
    ON CONFLICT (id) DO UPDATE SET
  ...
  const values = [config.baseUrl, config.username, config.password];
  const result = await pool.query(query, values);
  console.log('Thingsboard config saved successfully');
  return result.rows[0];
};

// Thingsboard設定の取得
const getThingsboardConfig = async () => {
  if (!pool) {
    throw new Error('データベースに接続されていません');
  }

  const query = 'SELECT * FROM thingsboard_configs ORDER BY created_at DESC LIMIT 1';
  const result = await pool.query(query);
  ...
  if (result.rows[0]) {
    ...
    console.log('Retrieved Thingsboard config:', {
      baseUrl: config.baseUrl,
      username: config.username,
      password: config.password ? '***' : 'empty'
    });
...
// Thingsboard接続テスト
const testThingsboardConnection = async (config) => {
  try {
    ...
    const response = await axios.post(url, requestBody, {
      ...
      validateStatus: function (status) {
        return status < 500; // 500未満のステータスコードを有効とする
      }
    });
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        ...
        throw new Error(`Thingsboard認証に失敗しました: ${error.response.status} ${error.response.statusText}`);
      }
    } else if (error.code === 'ECONNREFUSED') {
      throw new Error('Thingsboardサーバーに接続できません。URLを確認してください。');
    } else if (error.code === 'ENOTFOUND') {
      throw new Error('Thingsboardサーバーが見つかりません。URLを確認してください。');
    } else if (error.code === 'ETIMEDOUT') {
      throw new Error('Thingsboardサーバーへの接続がタイムアウトしました。');
    } else if (error.code === 'CERT_HAS_EXPIRED' || error.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE') {
      throw new Error('SSL証明書の問題が発生しました。サーバーの証明書を確認してください。');
    } else {
      throw new Error(`Thingsboard接続エラー: ${error.message}`);
    }
  }
};

// Thingsboardデバイスリスト取得
const getThingsboardDevices = async (config, pageSize = 20, page = 0) => {
  try {
    const axios = require('axios');
    
    // まず認証トークンを取得
    console.log('Getting Thingsboard authentication token...');
    const authUrl = `${config.baseUrl}/api/auth/login`;
    const authBody = {
      username: config.username,
      password: config.password
    };
    ...
    // デバイスリストを取得
    console.log('Fetching Thingsboard devices...');
    const devicesUrl = `${config.baseUrl}/api/tenant/devices?pageSize=${pageSize}&page=${page}`;
    ...
    if (devicesResponse.status === 200 && devicesResponse.data) {
      return {
        success: true,
        message: 'デバイスリストの取得に成功しました。',
        devices: devicesResponse.data.data || [],
        totalDevices: devicesResponse.data.totalElements || 0,
        totalPages: devicesResponse.data.totalPages || 0,
        hasNext: devicesResponse.data.hasNext || false
      };
    } else {
      throw new Error(`デバイスリストの取得に失敗しました: ${devicesResponse.status} ${devicesResponse.statusText}`);
    }
  } catch (error) {
    console.error('Thingsboard devices fetch failed:', error);
    console.error('Error details:', {
      message: error.message,
      code: error.code,
      response: error.response ? {
    ...
    if (error.response) {
      if (error.response.status === 401) {
        ...
        throw new Error('デバイスリストへのアクセス権限がありません。');
      } else if (error.response.status === 404) {
        throw new Error('デバイスAPIエンドポイントが見つかりません。');
      } else {
        throw new Error(`デバイスリスト取得エラー: ${error.response.status} ${error.response.statusText}`);
      }
    } else if (error.code === 'ECONNREFUSED') {
      throw new Error('Thingsboardサーバーに接続できません。URLを確認してください。');
    } else if (error.code === 'ENOTFOUND') {
      ...
      throw new Error(`デバイスリスト取得エラー: ${error.message}`);
    }
  }
};
...
module.exports = {
  ...
  getThingsboardConfig,
  testThingsboardConnection,
  getThingsboardDevices,
  getAssetByTopic,
  hasAssetForTopic,
